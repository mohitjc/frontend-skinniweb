...
const PaypalTransaction = () => {
  ...
  const getData = (p = {}) => {
    const accessToken = user.connectionsDetails.filter((item) => item.type === "paypal").map((item) => item.accessToken).pop();
    ...
    setLoader(true);
    ApiClient.get(shared.paypallist, filter).then((res) => {
      if (res.success) {
        setData(
          res.data?.map((itm) => {  
            return itm;
  ...
  const columns = [
    {
      ...
      name: "Date",
      render: (row) => {
        return <span className="">{datepipeModel.date(row?.updatedAt)}</span>;
      },
    },
    {
      key: "type",
    ...
    },
    {
      ...
      render: (row) => {
        return (
          <span className="">
            { pipeModel.currency(row?.subTotal)}
          </span>
        );
      },
    },
