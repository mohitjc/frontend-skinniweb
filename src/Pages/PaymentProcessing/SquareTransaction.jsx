...
const SquareTransaction = () => {
  ...
  const [filters, setFilter] = useState({ page: 1, count: 10, search: '',  start_date: startDate,
    end_date: endDate});
...
const refreshAccess = () => {
    const refreshAccessToken = user.connectionsDetails.filter((item) => item.type === "square").map((item) => item.refreshToken).pop();
    ApiClient.get("square/access_token_using_refresh_token", {refresh_token:refreshAccessToken}).then((res) => {
        if (res.success) {
        //   setData(
        //     res.data?.map((itm) => {  
        //       return itm;
  ...
  const getData = (p = {}) => {
    const accessToken = user.connectionsDetails.filter((item) => item.type === "square").map((item) => item.accessToken).pop();
    let filter = {token: accessToken, ...filters,...p };  
    if(filter.start_date) filter.start_date=`${filter.start_date}T00:00:00-0700`
    if(filter.end_date) filter.end_date=`${filter.end_date}T23:59:59-0700`
