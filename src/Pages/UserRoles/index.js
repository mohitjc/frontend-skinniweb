...
const UserRoles = () => {
  ...
  const {get:getBlog,isLoading,deleteApi,put,multiImageUpload}= ApiClientB();
  ...
  const getData = (p = {}) => {
    ...
    getBlog(shared.listApi, filter).then((res) => {
      if (res.success) {
        setData(
          res.data
            .map((itm) => {
  ...
  const deleteItem = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        loader(true);
        deleteApi(shared.deleteApi, { id: id }).then((res) => {
          if (res.success) {
            clear();
          }
          loader(false);
  ...
  const statusChange = (itm) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // loader(true);
        put(shared.statusApi, { id: itm.id, status }).then((res) => {
          if (res.success) {
            getData();
          }
          // loader(false);
  ...
  const edit = (id) => {
    ...
    history(`/${shared.url}/edit/${selectedItem}`);
  };

  const view = (id) => {
    const selectedItem = encryptUrlData(id)
    let url = `/${shared.url}/detail/${selectedItem}`;
    history(url);
  };
  ...
  const uploadFile = (e) => {
    ...
    if (!file) return;
    loader(true);
    multiImageUpload(url,files,{},'file').then((res) => {
      if (res.success) {
      }
      loader(false);
    });
